...
use vars qw{$err $errstr $drh $sqlite_version $spatialite_version};
...
BEGIN {
    ...
    $spatialite_version = undef;

}

__PACKAGE__->bootstrap($VERSION);

tie %COLLATION, 'DBD::Spatialite::_WriteOnceHash';
...
sub driver {
    ...
    if (!$methods_are_installed && $DBI::VERSION >= 1.608) {
        DBI->setup_driver('DBD::Spatialite');

        DBD::Spatialite::db->install_method('x_spatialite_last_insert_rowid');
        DBD::Spatialite::db->install_method('x_spatialite_busy_timeout');
        DBD::Spatialite::db->install_method('x_spatialite_create_function');
        DBD::Spatialite::db->install_method('x_spatialite_create_aggregate');
        DBD::Spatialite::db->install_method('x_spatialite_create_collation');
        DBD::Spatialite::db->install_method('x_spatialite_collation_needed');
        DBD::Spatialite::db->install_method('x_spatialite_progress_handler');
        DBD::Spatialite::db->install_method('x_spatialite_commit_hook');
        DBD::Spatialite::db->install_method('x_spatialite_rollback_hook');
        DBD::Spatialite::db->install_method('x_spatialite_update_hook');
        DBD::Spatialite::db->install_method('x_spatialite_set_authorizer');
        DBD::Spatialite::db->install_method('x_spatialite_backup_from_file');
        DBD::Spatialite::db->install_method('x_spatialite_backup_to_file');
        DBD::Spatialite::db->install_method('x_spatialite_enable_load_extension');
    ...
    $drh = DBI::_new_drh( "$_[0]::dr", {
        Name        => 'Spatialite',
        Version     => $VERSION,
        Attribution => 'DBD::Spatialite by Lokkju Brennr et al',
    } );
    return $drh;
}
...
sub connect {
    ...
    DBD::Spatialite::db::_login($dbh, $real, $user, $auth, $attr) or return undef;

    # Register the on-demand collation installer
    $DBI::VERSION >= 1.608
      ? $dbh->x_spatialite_collation_needed(\&install_collation)
      : $dbh->func(\&install_collation, "collation_needed");

    # Register the REGEXP function
    $DBI::VERSION >= 1.608
      ? $dbh->x_spatialite_create_function("REGEXP", 2, \&regexp)
      : $dbh->func("REGEXP", 2, \&regexp, "create_function");

    # HACK: Since PrintWarn = 0 doesn't seem to actually prevent warnings
    # in DBD::Spatialite we set Warn to false if PrintWarn is false.
...
sub install_collation {
    ...
    my $collation = $DBD::Spatialite::COLLATION{$collation_name}
        or die "can't install, unknown collation : $collation_name";
    $DBI::VERSION >= 1.608
        ? $dbh->x_spatialite_create_collation($collation_name => $collation)
...
sub prepare {
    ...
    DBD::Spatialite::st::_prepare($sth, $sql, @_) or return undef;

    return $sth;
}

sub _get_version {
    return ( DBD::Spatialite::db::FETCH($_[0], 'sqlite_version') );
}

sub _get_spatialite_version {
    return ( DBD::Spatialite::db::FETCH($_[0], 'spatialite_version') );
}

my %info = (
    17 => 'Spatialite',       # SQL_DBMS_NAME
...
#======================================================================
# An internal tied hash package used for %DBD::Spatialite::COLLATION, to
# prevent people from unintentionally overriding globally registered collations.

package DBD::Spatialite::_WriteOnceHash;
...
B<DBD::Spatialite> is a Perl DBI driver for iSpatialite enabled SQLite, that includes
...
Yes, B<DBD::Spatialite> is small and light, but it supports full transactions!
...
conflicting with B<DBD::Spatialite> and use the supplied C<sqlite3>
command line tool.
...
(released on Oct 14, 2009; bundled with DBD::Spatialite 1.26_05).
...
these are quite useful. DBD::Spatialite actually sets some (like
...
Returns the version of the SQLite library which B<DBD::Spatialite> is using,
e.g., "2.8.0". Can only be read.

=item sqlite_unicode

If set to a true value, B<DBD::Spatialite> will turn the UTF-8 flag on for all
...
=head2 $dbh->x_spatialite_last_insert_rowid()
...
C<$dbh-E<gt>x_spatialite_last_insert_rowid()> directly.

=head2 $dbh->x_spatialite_busy_timeout()

Retrieve the current busy timeout.

=head2 $dbh->x_spatialite_busy_timeout( $ms )

Set the current busy timeout. The timeout is in milliseconds.

=head2 $dbh->x_spatialite_create_function( $name, $argc, $code_ref )
...
without any implementation. The C<DBD::Spatialite> driver
...
=head2 $dbh->x_spatialite_create_collation( $name, $code_ref )
...
=head2 $dbh->x_spatialite_collation_needed( $code_ref )
...
L</"x_spatialite_create_collation">.

An initial callback is already registered by C<DBD::Spatialite>,
so for most common cases it will be simpler to just
add your collation sequences in the C<%DBD::Spatialite::COLLATION>
hash (see section L</"COLLATION FUNCTIONS"> below).

=head2 $dbh->x_spatialite_create_aggregate( $name, $argc, $pkg )
...
For more examples, see the L<DBD::Spatialite::Cookbook>.

=head2 $dbh->x_spatialite_progress_handler( $n_opcodes, $code_ref )
...
=head2 $dbh->x_spatialite_commit_hook( $code_ref )
...
C<x_spatialite_commit_hook> is overridden. A reference to the previous
...
=head2 $dbh->x_spatialite_rollback_hook( $code_ref )

This method registers a callback function to be invoked whenever a
transaction is rolled back. Any callback set by a previous call to
C<x_spatialite_rollback_hook> is overridden. A reference to the previous
callback (if any) is returned.  Registering an C<undef> disables the
callback.

=head2 $dbh->x_spatialite_update_hook( $code_ref )
...
C<x_spatialite_update_hook> is overridden. A reference to the previous
callback (if any) is returned.  Registering an C<undef> disables the
callback.
...
is an integer equal to either C<DBD::Spatialite::INSERT>,
C<DBD::Spatialite::DELETE> or C<DBD::Spatialite::UPDATE>
(see L</"Action Codes">);
...
=head2 $dbh->x_spatialite_set_authorizer( $code_ref )
...
authorizer callback should return C<DBD::Spatialite::OK> to allow the
action, C<DBD::Spatialite::IGNORE> to disallow the specific action but
allow the SQL statement to continue to be compiled, or
C<DBD::Spatialite::DENY> to cause the entire SQL statement to be rejected
...
=head2 $dbh->x_spatialite_backup_from_file( $filename )
...
=head2 $dbh->x_spatialite_backup_to_file( $filename )

This method accesses the SQLite Online Backup API, and will take a backup of
the currently connected database, and write it out to the named file.

=head2 $dbh->x_spatialite_enable_load_extension( $bool )
...
the C<DBD::Spatialite> module must be explicitly C<use>d at compile
...
The list of constants implemented in C<DBD::Spatialite> is given
...
In addition, C<DBD::Spatialite> automatically installs the
...
exposed through methods L</"x_spatialite_create_collation"> and
L</"x_spatialite_collation_needed">.

To avoid calling these functions every time a C<$dbh> handle is
created, C<DBD::Spatialite> offers a simpler interface through the
C<%DBD::Spatialite::COLLATION> hash : just insert your own
...
always grab the tied object underneath C<%DBD::Spatialite::COLLATION> ---
...
L<http://rt.cpan.org/NoAuth/ReportBug.html?Queue=DBD-Spatialite>
...
Lokkju Brennr E<lt>lokkju@lokkju.comE<gt>

=head1 COPYRIGHT
The bundled Spatialite code in this distribution is MPL/GPL/LGPL
...
Derived from L<DBD::SQLite> copyright 2002 - 2007 Matt Sergeant.
