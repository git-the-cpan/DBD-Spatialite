...
# Support code for DBD::Spatialite tests
...
C<< $dbh->x_spatialite_method_name(...) >>. This array helps to test
both ways. Usage :
...
C<< $dbh->x_spatialite_method_to_call(@args) >>.
...
# new_style, using $dbh->x_spatialite_*(...) --- starting from DBI v1.608
$DBI::VERSION >= 1.608 and push @CALL_FUNCS, sub {
  my $dbh       = shift;
  my $func_name = pop;
  my $method    = "x_spatialite_" . $func_name;
  return $dbh->$method(@_);
};
